...
function findAllPSiblings(where) {
    var nextSiblingP = [];
    var childNodes = where.children;

    for (let i = 0, len = childNodes.length - 1; i < len; i++) {  
        let curElem = childNodes[i];

        if (curElem.nextElementSibling !== null && curElem.nextElementSibling.tagName === 'P') {
            nextSiblingP.push(curElem);   
        }
    }
...
function deleteTextNodes(where) {
    ...
    textNodes.forEach(function(textElem) {
        textElem.remove();
    });
}
 ...
 * }
 */
function collectDOMStat(root, statObj) {
    ...
    for (let i = 0, len = childNodes.length; i < len; i++) { 
        ...
        if (curElem.nodeType === 1) {
            let tagName = curElem.tagName;

            if (statObj.tags === undefined) {
                statObj.tags = {};
                statObj.tags[tagName] = 1;   
            } else if (statObj.tags[tagName] === undefined) {
                statObj.tags[tagName] = 1;   
            } else {
                statObj.tags[tagName]++;   
            }  

            let classList = curElem.classList;

            if (classList !== undefined) {
                for (let i = 0, len = classList.length; i < len; i++) {
                    if (statObj.classes === undefined) {
                        statObj.classes = {};
                        statObj.classes[classList[i]] = 1; 
                    } else if (statObj.classes[classList[i]] === undefined) {
                        statObj.classes[classList[i]] = 1;   
                    } else {
        ...
        collectDOMStat(childNodes[i], statObj);   
    }
    ...
    var observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            ...
            if (mutation.removedNodes.length > 0) {
                ...
                nodesArr = [...mutation.removedNodes];

                returnObj.nodes = nodesArr;
            }
            if (mutation.addedNodes.length > 0) {
                returnObj.type = 'insert';
                nodesArr = [...mutation.addedNodes];
